...
namespace tagger {
  ...
  RadialSegmentSearch::findIntersectingChargeClusters( const std::vector<larcv::Image2D>& img_v,
    ...
    std::vector< std::vector<RadialHit_t > > planehits;
    for (int p=0; p<(int)img_v.size(); p++) {
      std::vector< RadialHit_t > radhits = findIntersectingChargeClusters( img_v[p], badch_v[p], thresholds[p], planerings[p] );
      planehits.emplace_back( std::move(radhits) );
    }
    return planehits;
  }

  std::vector< RadialHit_t > RadialSegmentSearch::findIntersectingChargeClusters( const larcv::Image2D& img, const larcv::Image2D& badch,
    ...
    std::vector<RadialHit_t> hitlist;
  ...
  std::vector< Segment2D_t > RadialSegmentSearch::make2Dsegments( const larcv::Image2D& img, const larcv::Image2D& badch, const std::vector<RadialHit_t>& hitlist,
    ...
    std::vector<int> img_coords = ublarcvapp::UBWireTool::getProjectedImagePixel( pos3d, img.meta(), 3 );
  ...
  std::vector< Segment3D_t > RadialSegmentSearch::find3Dsegments( const std::vector<larcv::Image2D>& img_v, const std::vector<larcv::Image2D>& badch_v,
    ...
    std::vector<int> origin_imgcoords = ublarcvapp::UBWireTool::getProjectedImagePixel( pos3d, img_v.front().meta(), 3 );
