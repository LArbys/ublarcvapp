...
namespace ublarcvapp {
  ...
  void UBWireTool::loadData() {
    ...
    TFile fGeoFile( Form("%s/larcv/app/PMTWeights/dat/geoinfo.root",getenv("LARCV_BASEDIR")), "OPEN" );
	...
	m_WireData[planeID] = ublarcvapp::WireData( planeID );
      }
    ...
    m_WireData[-1] = ublarcvapp::WireData( -1 );
  ...
  const ublarcvapp::WireData& UBWireTool::getWireData(int p) {
  ...
  void UBWireTool::pixelsAlongLineSegment( const std::vector<float>& start, const std::vector<float>& end, const std::vector<larcv::Image2D>& img_v,
					   const std::vector<float>& thresholds, const std::vector<int>& neighborhood, const float max_stepsize, std::vector< larcv::Pixel2DCluster >& pixels ) {
