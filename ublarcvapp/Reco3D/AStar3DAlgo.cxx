...
namespace reco3d {
  ...
  std::vector<AStar3DNode> AStar3DAlgo::findpath( const std::vector<larcv::Image2D>& img_v,
    ...
    ublarcvapp::UBWireTool::wireIntersection( start_wids, poszy_start, start_tri, start_crosses );

    double goal_tri = 0.;
    int goal_crosses = 0;
    std::vector< float > poszy_goal(2,0.0);
    ublarcvapp::UBWireTool::wireIntersection( goal_wids, poszy_goal, goal_tri, goal_crosses );
  ...
  */
  ...
  std::vector<larcv::Image2D> AStar3DAlgo::visualizeScores( std::string score_name, const std::vector<larcv::Image2D>& orig_img_v, Lattice& lattice ) {
  ...
  std::vector<AStar3DNode> AStar3DAlgo::downsampleAndFindPath( const int downsampling_factor, const std::vector<larcv::Image2D>& img_v,
    ...
    std::vector<AStar3DNode> out_path =  findpath( img_compressed_v, badch_compressed_v, badch_compressed_v, // tagged_compressed_v
                                                   start_rows_comp.front(), goal_rows_comp.front(), start_cols_comp, goal_cols_comp, goal_reached );
    
    // we translate points back to original resolution
    for ( auto& node : out_path ) {
      float nodetick = node.tyz[0];
      for ( size_t p=0; p<img_v.size(); p++ ) {
        auto const& orig_meta = img_v.at(p).meta();
        auto const& ds_meta   = img_compressed_v.at(p).meta();
        float nodewire = ds_meta.pos_x( node.cols[p] );
        node.cols[p] = orig_meta.col(nodewire);
        if ( p==0 ) node.row = orig_meta.row(nodetick); 
      }
    }
