...
namespace ublarcvapp {
  ...
  void ContourAStarCluster::addContour( int plane, const ContourShapeMeta* ctr, int idx ) {
    if (ctr==NULL) {
      std::stringstream ss;
  ...
  bool ContourAStarCluster::getCluster3DPointAtTimeTick( const int row, const std::vector<larcv::Image2D>& img_v,
	...
	ublarcvapp::UBWireTool::getMissingWireAndPlane( pta.plane, pta.maxqc, ptb.plane, ptb.maxqc, otherp, otherw, xsec_zy, crosses );
	if ( m_verbosity>1 )
  ...
  void ContourAStarClusterAlgo::extendSeedCluster( const std::vector<float>& pos3d, const std::vector<larcv::Image2D>& img_v,
    ...
    std::vector<int> seed_imgcoord = ublarcvapp::UBWireTool::getProjectedImagePixel( pos3d, img_v.front().meta(), img_v.size() );
    if ( m_verbosity>1 )
    ...
    // astar config ---------------------------
    reco3d::AStar3DAlgoConfig astar_cfg;
	...
	std::cout << " End:   (" << max_pos3d[0] << "," << max_pos3d[1] << "," << max_pos3d[2] << ")   " << std::endl;
      ...
      reco3d::AStar3DAlgo algo( astar_cfg );
	...
	break;
      ...
      if ( fMakeDebugImage ) {
	for ( auto &node : path ) {
	  std::vector<int> imgcoords = ublarcvapp::UBWireTool::getProjectedImagePixel( node.tyz, meta, img_v.size() );
	...
	v3d.push_back( v3 );
      ...
      min_imgcoords = ublarcvapp::UBWireTool::getProjectedImagePixel( v3d.front(), meta, img_v.size() );
      max_imgcoords = ublarcvapp::UBWireTool::getProjectedImagePixel( v3d.back(),  meta, img_v.size() );
  ...
  ContourAStarCluster ContourAStarClusterAlgo::makeSeedClustersFrom3DPoint( const std::vector<float>& pos3d, const std::vector<larcv::Image2D>& img_v,
    ...
    try {
      imgcoords = ublarcvapp::UBWireTool::getProjectedImagePixel( pos3d, img_v.front().meta(), img_v.size() );
    }
    catch (...) {
      std::stringstream ss;
  ...
  std::vector< std::set<int> >  ContourAStarClusterAlgo::extendClusterUsingAStarPath( ContourAStarCluster& cluster, std::vector< std::vector<float> >& path3d,
	...
	std::vector<int> imgcoordsmax = ublarcvapp::UBWireTool::getProjectedImagePixel( stepposmax, meta, img_v.size() );
	...
	// project back into the image	
	std::vector<int> imgcoordsmin = ublarcvapp::UBWireTool::getProjectedImagePixel( stepposmin, meta, img_v.size() );
  ...
  void ContourAStarClusterAlgo::fillInClusterImage( ContourAStarCluster& cluster, const std::vector< std::vector<float> >& path3d,
	...
	std::vector<int> imgcoords = ublarcvapp::UBWireTool::getProjectedImagePixel( pos, meta, img_v.size() );
